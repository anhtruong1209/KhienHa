...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(values) {
    try {
      ...
      if (!response.ok) {
        setError(response.data?.message || response.data?.error || "Đăng nhập thất bại.");
        return;
      }
      ...
      <div className="relative grid w-full max-w-5xl gap-6 lg:grid-cols-[0.94fr_1.06fr]">
        <div className="overflow-hidden rounded-[32px] bg-[#071b2f] p-8 text-white shadow-[0_28px_90px_rgba(7,27,47,0.22)]">
          ...
          <Text className="mt-4 block text-base leading-8 text-slate-300">
            Khu vực này chỉ dành cho người quản trị nội dung. Bạn cần nhập tài khoản và mật khẩu để vào CMS.
          </Text>
          ...
          <div className="mt-8 space-y-4">
            ...
            <div className="rounded-[24px] border border-white/10 bg-white/8 p-5">
              ...
              <div className="mt-3 text-sm leading-7 text-slate-200">
                Sau khi xác thực thành công, hệ thống sẽ lưu phiên bằng cookie và chặn truy cập admin nếu chưa đăng nhập.
              </div>
            </div>
          </div>
